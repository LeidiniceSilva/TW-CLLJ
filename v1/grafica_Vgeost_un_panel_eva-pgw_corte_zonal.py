...
__description__ = "This script plot geostrophic wind"
...
path = "/home/netapp-clima-users/users/jsalinas/Sim_RegCM/grad_geopot/Vgeostrofico"  # Path
...
def import_data(exp):
    ds = xr.open_dataset("{0}/prom-Vg925_grad_meridional-{1}.nc".format(path, exp))
    lats = ds["lat"]
    lons = ds["lon"]
    uwnd = ds["Vgeo"]
...
plt.ylabel("Geostrophic wind")
plt.title("Zonal mean 12.5°N–17.5°N")
plt.grid()
plt.legend()  # muestra los nombres con los mismos colores

plt.savefig("figs/Vgeostrof-corte-zonal-subplots-eva_pgw.png", dpi=400, bbox_inches="tight")
plt.show()
exit()
