...
path = '/home/netapp-clima-users/users/jsalinas/Sim_RegCM/efe_v' # Path
...
def import_data(exp):
        ds = xr.open_dataset('{0}/prom-efe_ve_{1}.nc'.format(path, exp))
	# print(ds)
...
plt.plot(lons[0,:], uwnd_mean1, label='Jun-Jul Evaluation')
plt.grid()
...
plt.plot(lons[0,:], uwnd_mean2, label='Aug-Oct Evaluation')
plt.grid()
...
plt.plot(lons[0,:], uwnd_mean3, label='Jun-Jul PGW')
plt.grid()
...
plt.plot(lons[0,:], uwnd_mean4, label='Aug-Oct PGW')
...
plt.ylabel('Coriolis fv')
plt.title('Zonal mean 12.5°N–17.5°N at 925 hPa')
plt.grid()
...
plt.savefig('figs/efe_ve-corte-zonal-subplots-eva_pgw.png', dpi=400, bbox_inches='tight')
