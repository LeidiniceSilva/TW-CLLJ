...
path = '/home/netapp-clima-users/users/jsalinas/Sim_RegCM/advectivos/grad_zonal' # Path
...
def import_data(exp):
        ds = xr.open_dataset('{0}/prom-u_du_meridional-{1}.nc'.format(path, exp))
	# print(ds)
...
# Smooth only between -63 and -59 longitude along the longitude axis (axes=-1)
lon_b1 = uwnd_band1['lon']
uwnd_smoothed1 = uwnd_band1.copy()
uwnd_smoothed1.values = gaussian_filter(uwnd_band1.values, sigma=4, axes=-1)
...
uwnd_smoothed2.values = gaussian_filter(uwnd_band2.values, sigma=4, axes=-1)
uwnd_band2 = xr.where((lon_b2 >= -63) & (lon_b2 <= -59), uwnd_smoothed2, uwnd_band2)
...
uwnd_smoothed3.values = gaussian_filter(uwnd_band3.values, sigma=4, axes=-1)
...
uwnd_smoothed4.values = gaussian_filter(uwnd_band4.values, sigma=4, axes=-1)
...
plt.ylabel('Advective zonal term')
plt.title('u_du_dx_zonal_mean 12.5°N–17.5°N at 925 hPa')
plt.grid()
plt.legend()  
...
plt.savefig('figs/advective-zonal-term-corte-zonal-subplots-eva_pgw.png', dpi=400, bbox_inches='tight')
plt.show()
exit()
