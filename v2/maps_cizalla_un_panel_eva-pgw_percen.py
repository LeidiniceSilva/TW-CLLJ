...
path = '/home/netapp-clima-users/users/jsalinas/Sim_RegCM/cizalla/con_850/percentil10' # Path
...
lats, lons, uwnd_jun_jul_80_17_eva = import_data('jun-jul-perc10-eva_con850')
lats, lons, uwnd_ago_oct_80_17_eva = import_data('ago-oct-perc10-eva_con850')
lats, lons, uwnd_jun_jul_80_17_pgw = import_data('jun-jul-perc10-pgw_con850')
lats, lons, uwnd_ago_oct_80_17_pgw = import_data('ago-oct-perc10-pgw_con850')
lats, lons, uwnd_jun_jul_diff = import_data('jun-jul-perc10-diff_con850')
lats, lons, uwnd_ago_oct_diff = import_data('ago-oct-perc10-diff_con850')
...
c_scheme = mp.pcolor(x,y,np.squeeze(uwnd_jun_jul_80_17_eva[0,:,:]), cmap='bwr', vmin=mi, vmax=md) # [0,:,:] is for the first day of the year
lon = np.array(x)
lat = np.array(y)
...
c_scheme = mp.pcolor(x,y,np.squeeze(uwnd_jun_jul_80_17_pgw[0,:,:]), cmap='bwr', vmin=mi, vmax=md) # [0,:,:] is for the first day of the year
lon = np.array(x)
lat = np.array(y)
...
c_scheme = mp.pcolor(x,y,np.squeeze(uwnd_ago_oct_80_17_eva[0,:,:]), cmap='bwr', vmin=mi, vmax=md) # [0,:,:] is for the first day of the year
lon = np.array(x)
lat = np.array(y)
...
c_scheme = mp.pcolor(x,y,np.squeeze(uwnd_ago_oct_80_17_pgw[0,:,:]), cmap='bwr', vmin=mi, vmax=md) # [0,:,:] is for the first day of the year
lon = np.array(x)
lat = np.array(y)
...
plt.savefig('figs/Cizalla-subplotseva-pgw_con850_perc10.png', dpi=400, bbox_inches='tight')
plt.show()
exit()
